...
extern int shared_keydown;

tlm::tlm_response_status
Gpio::read(const ensitlm::addr_t& a, ensitlm::data_t& d)
{
...
#ifdef DEBUG
		std::cout << name() << ": Read GPIO_DATA_OFFSET = " << std::hex << d << std::endl;
#endif
		break;
...
tlm::tlm_response_status
Gpio::write(const ensitlm::addr_t& a, const ensitlm::data_t& d)
{
	switch(a) {
	case GPIO_DATA_OFFSET:
		data = d;
