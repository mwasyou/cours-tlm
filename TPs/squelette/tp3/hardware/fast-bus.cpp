...
void FastBus::map(ensitlm::compatible_socket& port, ensitlm::addr_t start_addr, ensitlm::addr_t size) {
	port_map.insert(std::pair<ensitlm::compatible_socket*, addr_range>
			(&port, addr_range(start_addr, start_addr + size - 1)));
}

void FastBus::b_transport(tlm::tlm_generic_payload& trans, sc_core::sc_time& t) {
